...
uint16_t TurnRightExtraSpeed = 250; // 250
...
float Motor::RPMtoPWM(float TargetRPM, uint8_t Wheel)
{
  ...
  case LeftWheel:
    ...
    TargetPWM = (TargetRPM +563.14)*0.0175f; // need to test
...
void Motion::Forwards(uint16_t Speed)
{
  ...
  Motor::Moving_AntiClockwise(Speed + RightSpeedBias+100, RightWheel); //-20 -100
...
{
  /*If the two Wheels have bias, adjust one of the speed of the Wheel*/
  Motor::Moving_AntiClockwise(Speed + RightSpeedBias + TurnRightExtraSpeed+500, RightWheel); //+300
...
void Motion::Rightwards(uint16_t Speed)
{
  ...
  Motor::Moving_AntiClockwise(Speed + LeftSpeedBias + TurnLeftExtraSpeed + 850, LeftWheel); // 0//100
...
{
  /*If the two Wheels have bias, adjust one of the speed of the Wheel*/
  Motor::Moving_Clockwise(Speed + RightSpeedBias + TurnLeftExtraSpeed, RightWheel);   //-100
  Motor::Moving_AntiClockwise(Speed + LeftSpeedBias + TurnLeftExtraSpeed + 900, LeftWheel); // 0//100
  // Fix the Servo Motor to 45 deg all the time
  Servo::TurnDeg(180);
}
...
{
  /*If the two Wheels have bias, adjust one of the speed of the Wheel*/
  Motor::Moving_AntiClockwise(Speed + RightSpeedBias + TurnRightExtraSpeed+300, RightWheel); //+300
...
void Motion::Rightwards_135(uint16_t Speed)
{
  /*If the two Wheels have bias, adjust one of the speed of the Wheel*/
  Motor::Moving_Clockwise(Speed + RightSpeedBias + TurnLeftExtraSpeed-100, RightWheel);   //-100
  Motor::Moving_AntiClockwise(Speed + LeftSpeedBias + TurnLeftExtraSpeed + 5000, LeftWheel); // 0//100
  // Fix the Servo Motor to 45 deg all the time
  Servo::TurnDeg(90);
  // Servo::TurnDeg(135);
}
