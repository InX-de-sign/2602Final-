...
void LineTrackingTask(void *pvPara) {
  while (true) {
    ...
    //testing1: walking straight without linetracking
    // LineTracking::FollowingLine(OnTrack, LeftSpeed,RightSpeed);
  
    //testing2: linetracking
    //LineTracking::FollowingLine(IR::Tracking());
...
void calculateRPMTask(void *pvPara) {
  ...
  pid1.target_val = 15.0f;//left
  pid2.target_val = 15.0f;
  /*----------------------------------------------------*/
  while (true) {
    /*----------------------------------------------------*/
    /*FOR DEBUG USAGE*/
