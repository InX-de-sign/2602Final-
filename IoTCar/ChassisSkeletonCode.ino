...
{
  // initializes the JSON document(1024 bytes),
  // it contains two pieces of data: current_state and time_remain
  ...
  // Update the intersectionLights object (acts as a "explorer" and "collector" for traffic lights)
...
void printResult(AsyncResult &aResult)
{
  ...
  if (aResult.available())
  {
    ...
    if (RTDB.isStream())
    {
      ...
      // RTDB: firebase realtima databaase allowing real-time data streaming and processing
...
StackType_t uxRFIDTagReader[configMINIMAL_STACK_SIZE]; // provides stack space
StaticTask_t xRFIDTagReaderTCB;                        // maintains task metadata
TaskHandle_t RFIDTagReaderTCB;                         // allows task management
...
void RFIDTagReader(void *pvPara)
{
  ...
  while (true)
  {
    ...
    if (currenttagUID == "END_TTAG_UID")
    {
      LeftSpeed = RightSpeed = 0.0f;
      Database.set(aClient, "/demo/state", "stopped", asyncCB);
    }

    String path = "/rfid_logs/" + String(millis());
    Database.set(aClient, path.c_str(), tag.uid);

    Serial.print("debugging_RFID Tag: ");
    Serial.println(tag.uid);
...
void FireBaseTask(void *pvPara)
{
  ...
  while (true)
  {
    ...
    for (int i = 0; i < 10; i++)
    {
      auto &light = intersectionLights.intersectionLights[i];
